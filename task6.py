...
for linija in sve_linije: # prolazimo kroz svaku liniju fajla
    ...
    student_i_broj = linija_bez_praznina.split(',')  # .split ('.') ce razdvojiti ono sto je odvojeno zarezom
                                                    ...
                                                    # na nultoj poziciji je ime,a na prvoj broj casova kojima je stud. prisustvovao

    ime_studenta = str(student_i_broj[0]) # ime studenta je string
    broj_casova = int(student_i_broj[1])  # broj casova je cijeli broj

    if broj_casova < minimum: # za svaku od linija kroz koju prodjemo, ako je broj casova u toj liniji manji od minimalnog br casova
                        # koji student treba da ima da bi mogao izaci na ispit,uzecemo ime studenta iz te linije i dodati ga u litu
